...
function CardRoot({className, ...props}: CardRootProps) {
  return (
    <a href="/" className={twMerge("bg-navy-700 border-[0.5px] border-navy-600 p-3 space-y-4 rounded-lg block", className)} {...props} />
  );
}
...
function CardTitle({className, ...props}: CardTitleProps) {
  return (
    <span className={twMerge("text-small font-medium", className)} {...props}/>
  );
}
